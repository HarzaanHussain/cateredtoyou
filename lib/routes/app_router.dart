...
import 'package:cateredtoyou/models/user.dart'; // Importing User model
...
class AppRouter {
  ...
  late final GoRouter router = GoRouter(
    refreshListenable: authModel, // Listening to changes in authModel for refreshing routes
    ...
    routes: [
      GoRoute(
        ...
        builder: (context, state) => const LoginScreen(), // Building LoginScreen widget
        redirect: (context, state) {
          if (authModel.isAuthenticated) { // If user is authenticated
      ...
      GoRoute(
        ...
        builder: (context, state) => const RegisterScreen(), // Building RegisterScreen widget
        redirect: (context, state) {
          if (authModel.isAuthenticated) { // If user is authenticated
            return '/home'; // Redirect to home
          }
      ...
      GoRoute(
        ...
        builder: (context, state) => const HomeScreen(), // Building HomeScreen widget
        redirect: (context, state) {
          if (!authModel.isAuthenticated) { // If user is not authenticated
      ...
      GoRoute(
        ...
        builder: (context, state) => const StaffListScreen(), // Building StaffListScreen widget
      ),
      GoRoute(
        path: '/add-staff', // Path for add staff route
        builder: (context, state) => const AddStaffScreen(), // Building AddStaffScreen widget
      ),
      GoRoute(
        path: '/edit-staff', // Path for edit staff route
        builder: (context, state) {
          final staff = state.extra as UserModel; // Extracting UserModel from state
          return EditStaffScreen(staff: staff); // Building EditStaffScreen widget with staff data
        },
      ),
      GoRoute(
        path: '/inventory', // Path for inventory list route
        builder: (context, state) => const InventoryListScreen(), // Building InventoryListScreen widget
      ),
      GoRoute(
        path: '/add-inventory', // Path for add inventory route
        builder: (context, state) => const InventoryEditScreen(), // Building InventoryEditScreen widget
      ),
      GoRoute(
        path: '/edit-inventory', // Path for edit inventory route
        builder: (context, state) {
          final item = state.extra as InventoryItem; // Extracting InventoryItem from state
          return InventoryEditScreen(item: item); // Building InventoryEditScreen widget with item data
        },
      ),
    ],
