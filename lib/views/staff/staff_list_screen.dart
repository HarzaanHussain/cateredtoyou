...
class StaffListScreen extends StatefulWidget {
  ...
  @override
  State<StaffListScreen> createState() =>
      _StaffListScreenState(); // Creates the mutable state for this widget.
}
...
class _StaffListScreenState extends State<StaffListScreen> {
  ...
  List<UserModel> _filterStaff(List<UserModel> staffList) {
    if (_searchQuery.isEmpty) {
      return staffList; // If search query is empty, return the full list.
    }
    ...
    return staffList.where((staff) {
      ...
      return staff.fullName.toLowerCase().contains(
              searchLower) || // Check if full name contains search query.
          staff.email
              .toLowerCase()
              .contains(searchLower) || // Check if email contains search query.
          staff.role
              .toLowerCase()
              .contains(searchLower); // Check if role contains search query.
    }).toList();
  }
        ...
        children: [
            ...
            child: TextField(
              ...
              decoration: InputDecoration(
                ...
                prefixIcon: const Icon(
                    Icons.search), // Search icon inside the search field.
                border: OutlineInputBorder(
                  borderRadius: BorderRadius.circular(
                      10), // Rounded border for the search field.
                ),
                suffixIcon: _searchQuery
                        .isNotEmpty // Clear button if search query is not empty.
                    ? IconButton(
          ...
          Expanded(
            child: StreamBuilder<List<UserModel>>(
              stream: context
                  .read<StaffService>()
                  .getStaffMembers(), // Stream of staff members from the service.
              builder: (context, snapshot) {
                ...
                final allStaff =
                    snapshot.data ?? []; // Get the list of all staff members.
                final filteredStaff = _filterStaff(
                    allStaff); // Filter the staff list based on the search query.
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        ElevatedButton.icon(
                          ...
                          label:
                              const Text('Add Staff Member'), // Button label.
                        ),
                      ],
                    ),
                  );
                ...
                if (filteredStaff.isEmpty) {
                  return const Center(
                    child: Text(
                        'No staff members match your search'), // Message when no staff members match the search query.
                  );
                }
                ...
                return ListView.builder(
                  ...
                  itemCount:
                      filteredStaff.length, // Number of items in the list.
                  itemBuilder: (context, index) {
                    final staff = filteredStaff[
                        index]; // Get the staff member at the current index.
                    return StaffListItem(
                        staff:
                            staff); // Build the list item for the staff member.
                  },
                );
              },
            ),
...
class StaffListItem extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final bool isActive = staff.employmentStatus ==
        'active'; // Check if the staff member is active.
    final theme = Theme.of(context); // Get the current theme.
    final staffService =
        context.read<StaffService>(); // Get the staff service from the context.
    ...
    void toggleStatus() async {
      try {
        await staffService.changeStaffStatus(
          ...
          isActive
              ? 'inactive'
              : 'active', // Toggle the status between active and inactive.
        );
        ...
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                  'Error: $e'), // Show an error message if the status change fails.
    ...
    return Card(
      ...
      child: ListTile(
          contentPadding:
              const EdgeInsets.all(16), // Padding inside the list tile.
          onTap: () => context.push('/edit-staff',
              extra: staff), // Navigate to edit staff screen when tapped.
          leading: CircleAvatar(
            backgroundColor:
                theme.colorScheme.primary, // Background color of the avatar.
            child: Text(
              staff.firstName[0] +
                  staff.lastName[0], // Initials of the staff member.
              style: const TextStyle(
                  color: Colors.white), // Text style for the initials.
            ),
          ),
          title: Text(
                ...
                children: [
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: isActive
                          ? Colors.green.withAlpha((0.1 * 255)
                              .toInt()) // Background color for active status.
                          : Colors.red.withAlpha((0.1 * 255)
                              .toInt()), // Background color for inactive status.
                      borderRadius: BorderRadius.circular(
                          4), // Rounded corners for the status container.
                    ),
                    child: Text(
                      isActive
                          ? 'Active'
                          : 'Inactive', // Text indicating the status.
                      style: theme.textTheme.bodySmall?.copyWith(
                        color: isActive
                            ? Colors.green
                            : Colors.red, // Text color based on the status.
                        fontWeight:
                            FontWeight.bold, // Bold text for the status.
                      ),
                    ),
                  ),
                  TextButton(
                    ...
                    child: Text(isActive
                        ? 'Deactivate'
                        : 'Reactivate'), // Button text based on the status.
                  ),
                ],
              ),
            ],
          ...
          trailing: StreamBuilder<UserModel?>(
            ...
            builder: (context, snapshot) {
              ...
              final canManagePermissions =
                  ['admin', 'client', 'manager'].contains(currentUser.role);
