...
class DeliveryProgress {
  ...
  static double calculateProgress({
    ...
    final distanceProgress = _calculateDistanceProgress(waypoints, currentLocation, metadata); // Calculate distance progress
    ...
    return (distanceProgress * 0.6 + timeProgress * 0.4).clamp(0.0, 1.0); // Combine distance and time progress
  }
  
  /// Calculates progress based on distance traveled
  static double _calculateDistanceProgress(
    ...
    if (metadata != null && 
      ...
      final remainingDistance = metadata['routeDetails']['remainingDistance'] as num;
    ...
    if (totalDistance <= 0 || remainingDistance > totalDistance) {
      return 0.0;
    }
  ...
  static double _calculateTimeProgress(DateTime startTime, DateTime estimatedEndTime) {
    ...
    final totalDuration = estimatedEndTime.difference(startTime).inSeconds; // Total duration in seconds
    
    // Handle case where start and end times are the same or very close
    if (totalDuration <= 0) return 0.5;
    
    final elapsedDuration = now.difference(startTime).inSeconds; // Elapsed duration in seconds

    return (elapsedDuration / totalDuration).clamp(0.0, 1.0); // Return the progress as a fraction
  }
