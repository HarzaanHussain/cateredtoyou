...
import 'package:flutter_map_cancellable_tile_provider/flutter_map_cancellable_tile_provider.dart'; // Importing cancellable tile provider for map tiles

class DeliveryMap extends StatefulWidget {
  // Defining a stateful widget for the delivery map
  ...
  final List<Polyline>
      polylines; // List of polylines to be displayed on the map
  ...
  final bool
      showUserLocation; // Flag to indicate if user location should be shown

  const DeliveryMap({
    // Constructor for initializing the DeliveryMap widget
    super.key,
  ...
  @override
  State<DeliveryMap> createState() =>
      _DeliveryMapState(); // Creating the state for the DeliveryMap widget
}

class _DeliveryMapState extends State<DeliveryMap> {
  // State class for DeliveryMap
  @override
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) {
      // Adding a callback to be executed after the first frame
      if (widget.onMapReady != null) {
        // Checking if onMapReady callback is provided
  ...
  Widget build(BuildContext context) {
    if (widget.isLoading) {
      // Checking if the map is loading
      return const Center(
          child: CircularProgressIndicator()); // Displaying a loading indicator
    }

    return Stack(
      // Using a Stack to overlay widgets
      children: [
        FlutterMap(
          // Main map widget
          ...
          options: MapOptions(
            ...
            initialCenter:
                widget.initialPosition, // Setting the initial center position
          ...
          children: [
            TileLayer(
              ...
              userAgentPackageName: 'com.cateredtoyou.app', // User agent package name for the map tiles
              tileProvider: CancellableNetworkTileProvider(), // Using cancellable tile provider for better performance
              errorImage:
                  NetworkImage('https://via.placeholder.com/256?text=Error'),
              evictErrorTileStrategy: EvictErrorTileStrategy.dispose,
            ),
            if (widget
                .polylines.isNotEmpty) // Checking if there are any polylines
              PolylineLayer(
                  polylines: widget.polylines), // Adding polyline layer
          ...
          child: Column(
            ...
            children: [
              FloatingActionButton.small(
                ...
                heroTag: 'zoomIn',
                onPressed: () {
                  final currentZoom = widget.mapController.camera
                      .zoom; // Getting the current zoom level
                  widget.mapController.move(
                    // Moving the map to the new zoom level
  ...
  static Marker createMarker({
    ...
    return Marker(
      ...
      child: GestureDetector(
        // Adding tap detection to the marker
        onTap: onTap, // Setting the tap callback
        child: Stack(
          // Using a stack to overlay the icon and title
              ...
              Positioned(
                ...
                child: Container(
                  ...
                  padding: const EdgeInsets.symmetric(
                      horizontal: 4,
                      vertical: 2), // Adding padding to the title container
                  decoration: BoxDecoration(
                    color: Colors
                        .white, // Setting the background color of the title container
                    borderRadius: BorderRadius.circular(
                        4), // Adding rounded corners to the title container
                    boxShadow: [
                      BoxShadow(
                        color: Colors.black.withAlpha((0.2 * 255)
                            .toInt()), // Adding a shadow to the title container
                  ...
                  child: Text(
                    ...
                    style: TextStyle(
                      ...
                      fontWeight: FontWeight
                          .bold, // Setting the font weight of the title text
                    ),
                  ),
                ),
              ),
...
class MapBoundsHelper {
  static LatLngBounds calculateBounds(List<LatLng> points) {
    // Method to calculate the bounds of a list of points
    if (points.isEmpty) {
      // Checking if the points list is empty
      throw Exception(
          'Cannot calculate bounds for empty points list'); // Throwing an exception if the list is empty
    }
    ...
    for (var point in points) {
      ...
      if (point.latitude < minLat) {
        minLat = point.latitude; // Updating the minimum latitude
      }
      if (point.latitude > maxLat) {
        maxLat = point.latitude; // Updating the maximum latitude
      }
      if (point.longitude < minLng) {
        minLng = point.longitude; // Updating the minimum longitude
      }
      if (point.longitude > maxLng) {
        maxLng = point.longitude; // Updating the maximum longitude
      }
    }

    return LatLngBounds(
      // Returning the calculated bounds
  ...
  static LatLng calculateCenter(List<LatLng> points) {
    // Method to calculate the center of a list of points
    if (points.isEmpty) {
      // Checking if the points list is empty
      throw Exception(
          'Cannot calculate center for empty points list'); // Throwing an exception if the list is empty
    }
    ...
    return LatLng(
      // Returning the calculated center
