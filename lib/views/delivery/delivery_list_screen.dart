      ...
      body: Consumer<DeliveryRouteService>(
        builder: (context, deliveryService, child) {
          return StreamBuilder<List<DeliveryRoute>>(
            stream: deliveryService.getDeliveryRoutes(), // Stream of delivery routes.
              ...
              return ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return DeliveryRouteCard(
                    route: route,
                    onTap: () => context.push('/track-delivery', extra: route), // Navigate to track delivery screen.
                  );
                },
              );
            },
...
class DeliveryRouteCard extends StatelessWidget {
            ...
            children: [
              Row(
                ...
                children: [
                  ...
                  _buildStatusChip(context, route.status), // Display status chip.
                ],
              ),
              ...
              if (route.metadata?['notes'] != null) ...[
                ...
                Text(
                  route.metadata!['notes'], // Display delivery notes if available.
              ...
              if (showActions) ...[
                ...
                _buildActions(context), // Display action buttons if showActions is true.
              ],
            ],
          ),
        ),
  ...
  Widget _buildActions(BuildContext context) {
    if (!showActions) return const SizedBox.shrink(); // Return empty widget if showActions is false.

    final userId = FirebaseAuth.instance.currentUser?.uid; // Get current user ID.
    if (userId != route.driverId) return const SizedBox.shrink(); // Return empty widget if user is not the driver.
    ...
    return Row(
      ...
      children: [
        if (route.status == 'pending')
          TextButton.icon(
            onPressed: () => _updateStatus(context, 'in_progress'), // Start delivery.
            icon: const Icon(Icons.play_arrow),
            label: const Text('Start Delivery'),
          ),
        if (route.status == 'in_progress')
          TextButton.icon(
            onPressed: () => _updateStatus(context, 'completed'), // Complete delivery.
    ...
    try {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Error updating delivery status: ${e.toString()}'), // Show error message.
  ...
  Widget _buildTimeRow(BuildContext context) {
    ...
    final isDelayed =
        route.status != 'completed' && now.isAfter(route.estimatedEndTime); // Check if delivery is delayed.
        ...
        Expanded(
          child: Column(
            ...
            children: [
              ...
              Text(
                _formatTime(route.estimatedEndTime), // Display estimated end time.
                style: theme.textTheme.titleSmall?.copyWith(
                  color: isDelayed ? theme.colorScheme.error : null, // Highlight if delayed.
                ),
              ),
            ],
          ),
