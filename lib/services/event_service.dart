...
class EventService extends ChangeNotifier { // EventService class extending ChangeNotifier for state management.
  ...
  EventService(this._organizationService); // Constructor initializing OrganizationService.

  Stream<List<Event>> getEvents() async* { // Method to get a stream of events.
    try {
      final currentUser = _auth.currentUser; // Getting the current authenticated user.
      if (currentUser == null) { // If no user is authenticated, return an empty list.
        yield [];
        return;
      }

      final organization = await _organizationService.getCurrentUserOrganization(); // Getting the organization of the current user.
      if (organization == null) { // If no organization is found, return an empty list.
        yield [];
        return;
      }

      final query = _firestore // Querying Firestore for events belonging to the user's organization.
          .collection('events')
          .where('organizationId', isEqualTo: organization.id);

      yield* query.snapshots().map((snapshot) { // Mapping Firestore snapshots to a list of Event objects.
        try {
          final events = snapshot.docs
              .map((doc) => Event.fromMap(doc.data(), doc.id)) // Converting Firestore documents to Event objects.
              .toList();
          events.sort((a, b) => b.startDate.compareTo(a.startDate)); // Sorting events by start date in descending order.
          return events;
        } catch (e) {
          debugPrint('Error mapping events: $e'); // Printing error if mapping fails.
          return [];
        }
      });
    } catch (e) {
      debugPrint('Error in getEvents: $e'); // Printing error if fetching events fails.
      yield [];
    }
  }

  Future<void> _verifyInventoryAvailability(List<EventSupply> supplies) async { // Method to verify inventory availability.
    for (final supply in supplies) { // Iterating through each supply item.
      final doc = await _firestore
          .collection('inventory')
          .doc(supply.inventoryId)
          .get(); // Getting the inventory item document from Firestore.

      if (!doc.exists) { // If the inventory item does not exist, throw an error.
        throw 'Supply item ${supply.name} not found';
      }

      final item = InventoryItem.fromMap(doc.data()!, doc.id); // Converting Firestore document to InventoryItem object.
      if (item.quantity < supply.quantity) { // If the available quantity is less than required, throw an error.
        throw 'Insufficient quantity available for ${supply.name}. Available: ${item.quantity} ${item.unit}';
      }
    }
  }

  Future<void> _verifyStaffAssignment(String organizationId, List<AssignedStaff> staff) async { // Method to verify staff assignment.
    if (staff.isEmpty) return; // If staff list is empty, no need to verify.

    final currentUser = _auth.currentUser; // Getting the current authenticated user.
    if (currentUser == null) throw 'Not authenticated'; // If no user is authenticated, throw an error.

    final userDoc = await _firestore.collection('users').doc(currentUser.uid).get(); // Getting the user document from Firestore.
    final userRole = userDoc.data()?['role']; // Getting the user's role.

    if (!['client', 'manager', 'admin'].contains(userRole)) { // If the user does not have permission, throw an error.
      throw 'Insufficient permissions to assign staff';
    }

    for (final member in staff) { // Iterating through each staff member.
      final staffDoc = await _firestore.collection('users').doc(member.userId).get(); // Getting the staff member document from Firestore.

      if (!staffDoc.exists) { // If the staff member does not exist, throw an error.
        throw 'Staff member ${member.name} not found';
      }

      if (staffDoc.data()?['organizationId'] != organizationId) { // If the staff member belongs to a different organization, throw an error.
        throw 'Staff member ${member.name} belongs to a different organization';
      }

      if (staffDoc.data()?['employmentStatus'] != 'active') { // If the staff member is not active, throw an error.
        throw 'Staff member ${member.name} is not active';
      }

      final rolePermission = staffDoc.data()?['role']; // Getting the staff member's role.
      if (!['staff', 'server', 'chef', 'driver'].contains(rolePermission)) { // If the staff member has an invalid role, throw an error.
        throw 'Invalid staff role for ${member.name}';
      }
    }
  }

  Future<Event> createEvent({ // Method to create a new event.
    ...
    try {
      ...
      final userRef = _firestore.collection('users').doc(currentUser.uid); // Getting the user document reference.
      ...
      if (assignedStaff.isNotEmpty) { // If there are assigned staff, verify their assignments.
        await _verifyStaffAssignment(organization.id, assignedStaff);
        debugPrint('Staff assignments verified successfully'); // Printing debug message.
      }

      final staffData = assignedStaff.map((staff) => { // Formatting staff data with proper types.
        'userId': staff.userId,
        'name': staff.name,
        'role': staff.role,
      ...
      final totalPrice = menuItems.fold<double>( // Calculating the total price of menu items.
        0,
        (total, item) => total + (item.price * item.quantity),
      );

      return await _firestore.runTransaction<Event>((transaction) async { // Running a Firestore transaction to create the event.
        final customerRef = _firestore.collection('customers').doc(customerId); // Getting the customer document reference.
        final customerDoc = await transaction.get(customerRef); // Getting the customer document.
        ...
        await _verifyInventoryAvailability(supplies); // Verifying inventory availability.
        debugPrint('Inventory availability verified'); // Printing debug message.

        final docRef = _firestore.collection('events').doc(); // Creating a new event document reference.
        final now = DateTime.now(); // Getting the current date and time.

        final docData = { // Creating event data with proper Timestamps.
          'name': name.trim(),
        ...
        transaction.set(docRef, docData); // Setting the event document data in Firestore.
        debugPrint('Event document created'); // Printing debug message.

        for (final supply in supplies) { // Updating inventory for each supply item.
        ...
        debugPrint('Event creation completed successfully'); // Printing debug message.

        return Event( // Returning the created Event object.
          id: docRef.id,
          name: name.trim(),
          description: description.trim(),
          startDate: startDate,
      ...
      debugPrint('Error creating event: $e'); // Printing error if event creation fails.
      debugPrint('Stack trace: ${StackTrace.current}'); // Printing stack trace for debugging.
...
Future<void> updateEvent(Event updatedEvent) async {
  try {
    ...
    if (currentUser == null) throw 'Not authenticated'; // Throw error if user is not authenticated.

    debugPrint('Starting event update for ID: ${updatedEvent.id}'); // Log the start of the update process.
    ...
    final eventDoc = await _firestore.collection('events').doc(updatedEvent.id).get(); // Get the original event document.
    if (!eventDoc.exists) throw 'Event not found'; // Throw error if event is not found.
    final originalEvent = Event.fromMap(eventDoc.data()!, eventDoc.id); // Convert the document data to an Event object.

    if (updatedEvent.assignedStaff.isNotEmpty) {
      await _verifyStaffAssignment(organization.id, updatedEvent.assignedStaff); // Verify staff assignments.
    }
    ...
    debugPrint('Staff data being updated: $staffData'); // Log the staff data being updated.
    ...
    if (!listEquals(originalEvent.supplies, updatedEvent.supplies)) {
      ...
      await _firestore
          .collection('events')
          .doc(updatedEvent.id)
          .update(updateData); // If supplies didn't change, just update the event.
    }

    debugPrint('Event update completed successfully'); // Log the successful completion of the update.
  ...
  try {
    ...
    if (newStatus == EventStatus.completed || newStatus == EventStatus.cancelled) {
      await _firestore.runTransaction((transaction) async {
        ...
        if (!eventDoc.exists) throw 'Event not found'; // Throw error if event is not found.
        final event = Event.fromMap(eventDoc.data()!, eventDoc.id); // Convert the document data to an Event object.
