...
class RolePermissions extends ChangeNotifier {
  ...
  final FirebaseAuth _auth = FirebaseAuth
      .instance; // FirebaseAuth instance for authentication operations
  ...
  static const Map<String, Permission> allPermissions = {
    ...
    'manage_vehicles': Permission(
      id: 'manage_vehicles',
      name: 'Manage Vehicles',
      description: 'Create, edit, and manage fleet vehicles',
      category: 'Vehicle Management',
    ),
    'view_vehicles': Permission(
      id: 'view_vehicles',
      name: 'View Vehicles',
      description: 'View fleet vehicles',
      category: 'Vehicle Management',
    ),
    'manage_deliveries': Permission(
      id: 'manage_deliveries',
      name: 'Manage Deliveries',
      description: 'Create and manage delivery routes',
      category: 'Delivery Management',
    ),
    'view_deliveries': Permission(
      id: 'view_deliveries',
      name: 'View Deliveries',
      description: 'View delivery routes',
      category: 'Delivery Management',
    ),
    'manage_manifest': Permission(
      id: 'manage_manifest',
      name: 'Manage Manifests',
      description: 'Create, edit, and manage load manifests for deliveries',
      category: 'Delivery Management',
    ),
  };
  ...
  Future<bool> hasPermission(String permissionId) async {
    try {
      ...
      if (!doc.exists) {
        return false; // If the document does not exist, return false
      }

      final permissions = List<String>.from(doc.data()?['permissions'] ??
          []); // Get the list of permissions from the document
      return permissions.contains(
          permissionId); // Check if the specified permission is in the list
    } catch (e) {
      debugPrint(
          'Error checking permission: $e'); // Print an error message if an exception occurs
  ...
  List<String> getPermissionsForRole(String role) {
    return defaultRolePermissions[role] ??
        []; // Return the list of permissions for the specified role, or an empty list if the role is not found
  }

  // Create permissions document for a user
  Future<void> createUserPermissions(String uid, String role) async {
    try {
      final permissions = getPermissionsForRole(
          role); // Get the list of permissions for the specified role
      await _firestore.collection('permissions').doc(uid).set({
        ...
        'updatedAt': FieldValue
            .serverTimestamp(), // Set the updatedAt field to the current server timestamp
      });
    } catch (e) {
      debugPrint(
          'Error creating user permissions: $e'); // Print an error message if an exception occurs
    ...
    try {
      final permissions = customPermissions ??
          getPermissionsForRole(
              newRole); // Use custom permissions if provided, otherwise get the permissions for the new role
      await _firestore.collection('permissions').doc(uid).update({
        'permissions':
            permissions, // Update the permissions field in the document
        'role': newRole, // Update the role field in the document
        'updatedAt': FieldValue
            .serverTimestamp(), // Update the updatedAt field to the current server timestamp
      });
    } catch (e) {
      debugPrint(
          'Error updating user permissions: $e'); // Print an error message if an exception occurs
      rethrow; // Rethrow the exception to be handled by the caller
    }
  }
  ...
  Future<List<String>> getUserPermissions(String uid) async {
    try {
      ...
      if (!doc.exists) {
        return []; // If the document does not exist, return an empty list
      }

      return List<String>.from(doc.data()?['permissions'] ??
          []); // Return the list of permissions from the document
    } catch (e) {
      debugPrint(
          'Error getting user permissions: $e'); // Print an error message if an exception occurs
